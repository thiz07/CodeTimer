...
async function refreshDisplays() {
  try {
    ...
    setDebug(`OK: getDisplays() -> ${displays.length} display(s)`);
    ...
    if (lastState?.monitorId) els.monitorSelect.value = String(lastState.monitorId);
  } catch (e) {
    setDebug("ERROR: getDisplays failed -> " + (e?.message || String(e)));
  }
}
